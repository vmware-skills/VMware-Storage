"""Interactive first-run setup wizard — ``vmware-storage init``.
...
from vmware_storage.config import (
    CONFIG_DIR,
    CONFIG_FILE,
    ENV_FILE,
...
def run_init(force: bool = False, skip_test: bool = False) -> int:
    ...
    console.print("[bold cyan]vmware-storage init[/] — guided setup\n")
    ...
    if skip_test:
        console.print("\nNext: [cyan]vmware-storage doctor[/] to verify the connection.")
        return 0

    if not typer.confirm("\nTest the connection now?", default=True):
        console.print("Next: [cyan]vmware-storage doctor[/] to verify the connection.")
        return 0
