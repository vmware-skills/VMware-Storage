"""Regression tests for onboarding: the `vmware-storage init` wizard, the doctor
...
from vmware_storage import init_wizard
...
def _wizard_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    cfg_dir = tmp_path / ".vmware-storage"
...
def test_init_writes_grep_safe_env(_wizard_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from vmware_storage.config import _decode_secret
...
def test_doctor_init_reference_is_backed_by_real_command():
    from vmware_storage import doctor

    src = Path(doctor.__file__).read_text()
    if "vmware-storage init" in src:
...
def test_invalid_login_error_is_teaching(capsys):
    ...
    from vmware_storage.cli import handle_cli_errors
    ...
    assert ".vmware-storage/.env" in out
