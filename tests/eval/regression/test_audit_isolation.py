...
``vmware_storage`` is imported: ``HOME`` and ``OPS_HOME`` both point into a
...
database; it does *not* cover ``vmware_storage.notify.audit.AuditLogger``, whose
default path is the ``~``-relative string ``~/.vmware-storage/audit.log`` and
...
def test_the_skill_json_lines_log_is_inside_the_sandbox() -> None:
    ...
    from vmware_storage.cli import _audit as cli_audit
...
def test_an_audited_write_lands_in_the_sandbox_and_not_in_the_real_db() -> None:
    ...
    get_engine().log(skill="storage", tool=marker, params={}, result={}, status="ok")
