...
def _load_server(monkeypatch, read_only: str | None):
    """Import vmware_storage.mcp_server.server fresh under the given read-only env."""
    monkeypatch.delenv("VMWARE_READ_ONLY", raising=False)
    monkeypatch.delenv("VMWARE_STORAGE_READ_ONLY", raising=False)
    if read_only is not None:
        monkeypatch.setenv("VMWARE_READ_ONLY", read_only)

    for name in [m for m in sys.modules if m.startswith("vmware_storage.mcp_server")]:
        del sys.modules[name]
    return importlib.import_module("vmware_storage.mcp_server.server")
...
def test_read_only_keeps_read_tools(monkeypatch):
    ...
    server = _load_server(monkeypatch, "true")
    names = _tool_names(server)
    for tool in ("list_all_datastores", "browse_datastore", "storage_iscsi_status", "vsan_health"):
...
def test_skill_env_var_also_works(monkeypatch):
    monkeypatch.delenv("VMWARE_READ_ONLY", raising=False)
    monkeypatch.setenv("VMWARE_STORAGE_READ_ONLY", "true")
    for name in [m for m in sys.modules if m.startswith("vmware_storage.mcp_server")]:
        del sys.modules[name]
    server = importlib.import_module("vmware_storage.mcp_server.server")
