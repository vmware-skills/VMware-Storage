...
vmware-policy cannot read this skill's config itself, so ``vmware_storage/mcp_server/server.py``
...
from vmware_storage.config import AppConfig, TargetConfig

import vmware_storage.mcp_server.server as server
...
def stub_vcenter(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "enable_software_iscsi", lambda si, host_name: f"iSCSI enabled on {host_name}"
    )
    monkeypatch.setattr(
        server, "list_datastores", lambda si: paginated([{"name": "ds-01"}], total=1)
    )
    monkeypatch.setattr(server, "_safe_audit", lambda **kwargs: None)
...
def test_write_against_undeclared_target_warns_but_runs(monkeypatch, stub_vcenter):
    ...
    _declare(monkeypatch, "")
    result = server.storage_iscsi_enable(host_name="esxi-01", target="prod-vcenter")
    assert result == "iSCSI enabled on esxi-01"
...
def test_write_against_undeclared_target_is_denied_when_enforcing(
    ...
    with pytest.raises(PolicyDenied) as excinfo:
        server.storage_iscsi_enable(host_name="esxi-01", target="prod-vcenter")
...
def test_write_against_declared_lab_target_succeeds(
    monkeypatch, enforcing, stub_vcenter
):
...
def test_read_against_undeclared_target_works(monkeypatch, stub_vcenter):
    ...
    _declare(monkeypatch, "")
    assert server.list_all_datastores(target="prod-vcenter")["items"] == [{"name": "ds-01"}]
