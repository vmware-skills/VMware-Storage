...
Installed at *import* time, not in a fixture. ``vmware_storage.cli`` and
``vmware_storage.mcp_server.server`` each build an ``AuditLogger()`` at module
scope, and that constructor expands ``~/.vmware-storage/audit.log`` and creates
the directory there and then. A fixture — even a session-scoped autouse one —
runs after collection has already imported every test module and, with them,
those modules. By the time a fixture could redirect ``HOME``, the path is fixed.
...
* ``HOME`` moves ``~/.vmware-storage/audit.log``, the per-skill JSON Lines log,
  whose default path is a ``~``-relative string and so ignores ``OPS_HOME``.

Before this existed, ``pytest`` appended rows to the operator's live
``~/.vmware/audit.db``. An audit trail that contains test fiction cannot answer
the question it is kept to answer — a reader cannot tell the invented rows from
the real ones. The sibling finding in VMware-VKS was this same leak with louder
rows (``delete_tkc_cluster {"confirmed": true}`` for a cluster that never
existed); the leak belongs to the family, not to the noisy repo.
...
SANDBOX_HOME = Path(tempfile.mkdtemp(prefix="vmware-storage-tests-"))
